    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope('conv_sub_1-1') as scope:
                self.W1_con_sub = tf.get_variable(name='W1_con_sub', shape = [5, 5, 3, 40], dtype=tf.float32,
            ...
            with tf.name_scope('conv_sub_1-2') as scope:
                self.W2_con_sub = tf.get_variable(name='W2_con_sub', shape = [5, 5, 40, 80], dtype=tf.float32,
            ...
            with tf.name_scope('conv_layer2') as scope:
                self.W3_con_layer = tf.get_variable(name='W3_con_layer', shape= [3, 3, 80, 160], dtype=tf.float32,
                                                    initializer=tf.contrib.layers.variance_scaling_initializer())
            ...
            with tf.name_scope('conv_layer3') as scope:
                self.W4_con_layer = tf.get_variable(name='W4_con_layer', shape= [3, 3, 160, 320], dtype=tf.float32,
                                                    initializer=tf.contrib.layers.variance_scaling_initializer())
                ...
                self.L4_con_layer = tf.reshape(self.L4_con_layer, [-1, 4*4*320])
            ############################################################################################################
            ## �� fully connected ���� - 1
            ...
            with tf.name_scope('fc_layer1') as scope:
                self.W_fc1 = tf.get_variable(name='W_fc1', shape=[4*4*320, 600], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b_fc1 = tf.Variable(tf.constant(value=0.001, shape=[600], name='b_fc1'))
            ...
            with tf.name_scope('fc_layer2') as scope:
                self.W_fc2 = tf.get_variable(name='W_fc2', shape=[600, 800], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b_fc2 = tf.Variable(tf.constant(value=0.001, shape=[800], name='b_fc2'))
            ...
            ##  �� Ȱ��ȭ �Լ� �� Softmax
            ############################################################################################################
            self.W_out = tf.get_variable(name='W_out', shape=[800, 2], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
