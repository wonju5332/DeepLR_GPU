    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope('conv_sub_1') as scope:
                ...
                self.L1_sub1_1 = tf.layers.conv2d(inputs=self.X, filters=32, kernel_size=[3, 3],
                                     kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
                                     padding="SAME", activation=None, name='L1_con_sub_1_1')
            ...
            with tf.name_scope('conv_layer1') as scope:
                ...
                # self.W3_con_layer = tf.get_variable(name='W3_con_layer', shape= [3, 3, 50, 120], dtype=tf.float32,
                #                                     initializer=tf.contrib.layers.variance_scaling_initializer())
            ...
            with tf.name_scope('fc_layer2') as scope:
                self.W_fc2 = tf.get_variable(name='W_fc2', shape=[500, 500], dtype=tf.float32,
                                             initializer=tf.contrib.layers.variance_scaling_initializer())
            ...
            ##  �� Ȱ��ȭ �Լ� �� Softmax
            ############################################################################################################
            self.W_out = tf.get_variable(name='W_out', shape=[500, 2], dtype=tf.float32,
                                         initializer=tf.contrib.layers.variance_scaling_initializer())
