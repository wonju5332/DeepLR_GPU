...
class Model:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name): #아래의 Variable 들을 관리한다.
            with tf.name_scope('input_layer') as scope:
                ...
                self.X = tf.placeholder(tf.float32, [None, 784], name='x_data')   # [None, 4096]
            ...
            ##  ⊙ 드롭 아웃 구현
            ############################################################################################################
            with tf.name_scope('conv_layer1') as scope:   #shape = [fxf , input채널, ouput채널]
                self.W1 = tf.get_variable(name='W1', shape=[3, 3, 1, 32], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer()) # He (가중치초기화)
                self.b1 = tf.Variable(tf.constant(value=0.001, shape=[32]), name='b1') # 편향을 0.001로 (,32)형태로 만들어놓는다.
                self.L1 = tf.nn.conv2d(input=X_img, filter=self.W1, strides=[1, 1, 1, 1], padding='SAME') # 28x28 -> 28x28  패딩개념을 넣어서, size같게
                # self.L1 = tf.nn.relu(self.L1, name='R1')
                self.L1 = self.parametric_relu(self.L1, 'R1')
                # self.L1 = self.exp_relu(self.L1+self.b1, 'R1')
                self.L1 = tf.nn.max_pool(value=self.L1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 28x28 -> 14x14
                self.L1 = tf.layers.dropout(inputs=self.L1, rate=self.dropout_rate, training=self.training)  #training에는 bool이 들어감.
            ...
            with tf.name_scope('conv_layer2') as scope:
                ...
                self.L2 = tf.nn.conv2d(input=self.L1, filter=self.W2, strides=[1, 1, 1, 1], padding='SAME') # 14x14 -> 14x14
                # self.L2 = tf.nn.relu(self.L2, name='R2')
                self.L2 = self.parametric_relu(self.L2, 'R2')
                # self.L2 = self.exp_relu(self.L2+self.b2, 'R2')
            ...
            with tf.name_scope('conv_layer3') as scope:
                ...
                self.L3 = tf.nn.conv2d(input=self.L2, filter=self.W3, strides=[1, 1, 1, 1], padding='SAME') # 7x7 -> 7x7
                # self.L3 = tf.nn.relu(self.L3, name='R3')
                self.L3 = self.parametric_relu(self.L3, 'R3')
                # self.L3 = self.exp_relu(self.L3+self.b3, 'R3')
            ...
            with tf.name_scope('conv_layer4') as scope:
                ...
                # self.L4 = self.exp_relu(self.L4+self.b4, 'R4')
            ...
            with tf.name_scope('fc_layer1') as scope:
                ...
                # self.L5 = self.exp_relu(tf.matmul(self.L4, self.W5)+ self.b5, 'R5')
            ...
            with tf.name_scope('fc_layer2') as scope:
                ...
                # self.L6 = self.exp_relu(tf.matmul(self.L5, self.W6) + self.b6, 'R6')
        ...
        ##  ⊙ λ/(N)*Σ|W|
        ################################################################################################################
        weightDecay_L2 = (0.01/(2*tf.to_float(tf.shape(self.X)[0])))*tf.reduce_sum(tf.square(self.W7))
        weightDecay_L1 = (0.001 / tf.to_float(tf.shape(self.X)[0])) * tf.reduce_sum(tf.abs(self.W7))
        self.cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=self.Y)) + weightDecay_L2
    ...
    # def leaky_relu(self, _x, name):
    #     return tf.maximum(_x, _x*0.1)

    # def exp_relu(self, _x,name):
    #     return tf.nn.elu(_x)
...
test_size = len(mnist.test.labels) # 10000
predictions = np.zeros(test_size * 10).reshape(test_size, 10)  # (10000, 10)

model_result = np.zeros(test_size*2, dtype=np.int).reshape(test_size, 2) # (10000, 2)
